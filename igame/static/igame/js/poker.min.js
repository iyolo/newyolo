...
(function() {
	if(window.CanvasRenderingContext2D) {
		...
		c.svgCurve = function(J, I, L, A) {
			var B = this,
				G, F, C, D, H, E, z = function(y) {
					return(G = J + y * L / p)
				},
				K = function(y) {
					return(F = I + y * L / p)
				};
			H = A.replace(/ *([MZLHVCSQTA]) */gi, "|$1,").replace(/^\||\|[Z],/gi, "").split(/\|/);
			B.beginPath();
			for(C in H) {
				D = H[C].split(/[, ]/);
				E = D[0];
				(E === "M") ? (B.moveTo(z(D[1]), K(D[2]))) : (E === "L") ? (B.lineTo(z(D[1]), K(D[2]))) : (E === "H") ? (B.lineTo(z(D[1]), F)) : (E === "V") ? (B.lineTo(G, K(D[1]))) : (E === "C") ? (B.bezierCurveTo(z(D[1]), K(D[2]), z(D[3]), K(D[4]), z(D[5]), K(D[6]))) : (E === "Q") && (B.quadraticCurveTo(z(D[1]), K(D[2]), z(D[3]), K(D[4])))
			}
			B.closePath()
		};
